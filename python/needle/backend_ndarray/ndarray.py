...
class NDArray:
    ...
    def permute(self, new_axes):
        ...
        Returns:
            ...
            strides changed).
        eg: array shape (5, 2, 5, 8), axes = (0, 2, 3, 1), strides = (80, 40, 8, 1)
            new   shape (5, 5, 8, 2),                  new_strides = (80, 8, 1, 40)
        """
    ...
    def __getitem__(self, idxs):
        ...
        new_strides = [0 for _ in range(self.ndim)]
        for i in range(self.ndim):
            new_shape[i] = int((idxs[i].stop - idxs[i].start) / idxs[i].step)
            if (idxs[i].stop - idxs[i].start) % idxs[i].step != 0:
                new_shape[i] += 1
    ...
    def reduce_view_out(self, axis):
        ...
        if axis is None:
            ...
            """
            original array shape: (m, n, j, k, h); axis = 3
            view array : permute(0, 1, 2, 4, 3)=>(m, n, j, h, k), out array shape: (m, n, j, h, 1)
            when an array is permuted, the new array may not be compact
            """
            view = self.permute(
